...
app.get('/itemSelectInvGroup',function(req,res){
     
   // db.user.find(function(err,doc){ req.body.InvGroupName
    
    db.items.find({"InvGroupName" : req.query.InvGroupName },function(err,doc){

       console.log("  itemSelectInvGroup   itemSelectInvGroup    "+doc.length) 
        res.json(doc);
    })
})
...
app.put('/putDeliverd/:thh',function(req,res){
  console.log("ppppppppppppppppppppp000000000000000042222222222222222")
var id=req.params.thh;
  
   console.log(id+"pppppppppppppppppppppppppppppp222222222222228888888")

    db.orders.update({"orderNO" :id,"initial":"Completed"},{$set:{"initial":"Delivered" }},function(err,doc)
        {
   ...
   db.orderManage.update({"orderNO" :id,"initial":"Completed"},{$set:{"initial":"Delivered" }},function(err,doc)
        {
          // console.log(doc.name+"aaaaaaaaaaaaaaaaaaaaaaaa");
         // console.log(doc); "SalesTax":req.body.salesTax,
            //res.json(doc);
...
app.get('/saleNames',function(req,res){
   ...
   // db.user.find(function(err,doc){
    db.staff.find({},function(err,doc){
        ...
        console.log(doc)
        res.json(doc);
        console.log("iiiiiiiiiiiiiiiiiiiiioooooooooooooooooooooooooo"+doc)
    })
})
app.get('/ordNum/:orderNum',function(req,res){
    var ordeenum = req.params.orderNum;
    //var compositenum = req.query.compositenum;
    db.orders.aggregate([
{$match:{"orderNO":ordeenum,"initial":"Completed"}},
 ...
 //   // console.log(str);
  ...
  console.log(a+c+"jjjjjjjjjjjjjjjjjjjoooooooooooooooggggggggggggggggggggg")
   db.orders.find({"partyNames":a,"orderNO":c},function(err,doc){     
        ...
        res.json(doc);
        console.log(doc.length+"   kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk")
...
app.post("/receiptVoucherSale:datareceipt",function(req,res){
  ...
  console.log(datastr+"data data data data data data data data");
  var datastr_array=datastr.split(",");
  var pname=datastr_array[0];
  var tran=datastr_array[1];
  var vNo=datastr_array[2];
  var taxableval=datastr_array[3];
  var taxamt=datastr_array[4];
  var subtol=datastr_array[5];
  var invoice=datastr_array[6];
  var netamt=datastr_array[7];
  var date=datastr_array[8];
  var status="completed";
  db.saleInvoice.insert({"partyname":pname,"Transaction":tran,"voucherNo":vNo,"taxableval":taxableval,"tax":taxamt,
          "subtol":subtol,"invoiceValue":invoice,"netamt":netamt,"date":date,"status":status},function(err,doc){
            res.json(doc);
            console.log(doc);
          });
});
...
app.get('/ddpctOrder',function(req,res){
   ...
   // db.user.find(function(err,doc){
    db.pct.find({"type" : "Sale"},function(err,doc){
...
// app.put('/completedChange',function(req,res){
//   //  console.log(name+"1111111111111111111111111111111"){"_id":mongojs.ObjectId(id)};
//      db.orderManage.update({_id : mongojs.ObjectId(id)},{$set:{"allocate":req.body.allocate,"initial":req.body.initial }},function(err,doc)
//         {
//           // console.log(doc.name+"aaaaaaaaaaaaaaaaaaaaaaaa");
...
app.get('/getOrderNum/:name',function(req,res)
{
  console.log(req.params.name+"gggggggghhhhhhhhhhhhopppppppppppppppppqqqqqqqqqqqqqqqqqqqqqqqq")
...
app.get('/ordNoFromOrders/:partyname',function(req,res){
  
   pname=req.params.partyname
    console.log(pname+"rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr");
    db.orders.aggregate([{$match:{"initial":"Completed","partyNames":pname}},
      {$group:{_id:{orderNO:"$orderNO"}}}],function(err,doc){
      console.log(doc+"ppppppppppppppppppppppppppppppppppppppp")  
          res.json(doc);
      });
    // db.orders.find({"initial":"Completed","partyNames":pname},function(err,doc1){
...
app.put('/initialSaveIssueVocher/:thh',function(req,res){
  console.log("ppppppppppppppppppppp000000000000000042222222222222222")
...
var str=req.params.thh;
   ...
   console.log(id+"pppppppppppppppppppppppppppppp222222222222228888888")
      ...
      //  var name=req.body.Name;
      //  console.log(name+"1111111111111111111111111111111"){"_id":mongojs.ObjectId(id)};
     db.orders.update({_id : mongojs.ObjectId(id)},{$set:{"initial":"Allocated","allocate":allocate }},function(err,doc)
        {
          // console.log(doc.name+"aaaaaaaaaaaaaaaaaaaaaaaa");
         // console.log(doc); "SalesTax":req.body.salesTax,
            res.json(doc);
...
app.put('/initialSaveReceiptVocher/:thh',function(req,res){
  console.log("ppppppppppppppppppppp000000000000000042222222222222222")
...
var str=req.params.thh;
   ...
   db.orders.update({_id : mongojs.ObjectId(id)},{$set:{"initial":"Received","allocate":allocate }},function(err,doc)
        {
       res.json(doc);
...
//for getting the barcode details to reprint the barcode
app.get('/barcodereprint:barcode1',function(req,res){
  console.log("reprinting the barcode through the print button");
...
app.get('/staffouts:data',function(req,res){
  console.log("getting approval out's for staff");
  var staff=req.params.data;
  var trans="Approval Out";
  console.log(staff+" selected staffname ");
  db.transactionDetail.aggregate([{$match:{"StaffName":staff,"Transaction":trans,"voucherNo":{$ne:"null"}}},
      {$group:{_id:{voucherNo:"$voucherNo"}}}],function(err,doc){ 
    res.json(doc);
    console.log(doc+"Approval Outs");
  });
  ...
  if( wt == "undefined" || wt ==  0){
       ...
       if(tran == "Regular Sale"){ //"stockInward":stockInward,
         db.transactionDetail.insert({"Transaction":tran,"barcodeNumber":bar,"chgunt":chgunt,"date":date,"desc":desc,"final":final,"gpcs":gpcs,"gwt":gwt,
                ...
                // console.log(doc); ,"orderNo":orderNo   
        })
...
app.get('/adminStaffDetails',function(req,res){  

db.staff.count({"desgination" : "Admin"},function(err,doc){
    console.log(doc);
    res.json(doc);
  });
...
app.post('/insertStaffDetails',function(req,res){


   db.staff.insert(req.body,function(err,doc){
       res.json(doc);
   })
})
app.post('/updateStaffDetails',function(req,res){

   db.staff.update({_id:mongojs.ObjectId(req.body._id)},{$set:{name:req.body.name,password:req.body.password,desgination:req.body.desgination}},function(err,doc){
       res.json(doc);
   })
})

app.delete('/deleteStaffDetails',function(req,res){
   console.log(req.query._id)
    db.staff.remove({_id: mongojs.ObjectId(req.query._id)}, function(err, docs) {
        res.json(docs);
    })
})

//start saleCategory cntrl
app.get('/getSaleCategoryCount',function(req,res){  
...
app.get('/editSaleDetails',function(req,res){  

  db.salesCategoryMaster.find({"_id":mongojs.ObjectId(req.query._id)},function(err,doc){
      res.json(doc);
  })
})
app.post('/insertSaleDetails',function(req,res){


   db.salesCategoryMaster.insert(req.body,function(err,doc){
       res.json(doc);
   })
})
app.post('/updateSaleDetails',function(req,res){

   db.salesCategoryMaster.update({_id:mongojs.ObjectId(req.body._id)},{$set:{SaleCategoryNo:req.body.SaleCategoryNo,SaleCategoryType:req.body.SaleCategoryType,SortedOrderNo:req.body.SortedOrderNo,PrnFileName:req.body.PrnFileName}},function(err,doc){
       res.json(doc);
   })
})

app.delete('/deleteSaleDetails',function(req,res){
   console.log(req.query._id)
    db.salesCategoryMaster.remove({_id: mongojs.ObjectId(req.query._id)}, function(err, docs) {
        res.json(docs);
    })
})
//end saleCategory cntrl
//start InventoryGroup cntrl
app.get('/getInventoryGroupCount',function(req,res){  
...
app.get('/editinventoryGroupDetails',function(req,res){  

  db.inventoryGroupMaster.find({"_id":mongojs.ObjectId(req.query._id)},function(err,doc){
      res.json(doc);
  })
})
app.post('/insertinventoryGroupDetails',function(req,res){


   db.inventoryGroupMaster.insert(req.body,function(err,doc){
       res.json(doc);
   })
})
...
app.delete('/deleteinventoryGroupDetails',function(req,res){
   console.log(req.query._id)
    db.inventoryGroupMaster.remove({_id: mongojs.ObjectId(req.query._id)}, function(err, docs) {
        res.json(docs);
    })
})
...
var id =req.query.id;
       console.log(id+"hihihihoijihihihihiiiiihio");
    ...
    //db.transactionDetail.find({"partyname":partyname,"Transaction":trans},function(err,doc){
      db.transactionDetail.find({_id:mongojs.ObjectId(id)},function(err,doc){
         console.log(doc+"bbbbbbbbbbbbbbbbb");
        res.json(doc);
        // console.log(doc);
...
//get Order no
app.get('/Orderprefixs',function(req,res ){
  console.log("ooooooooooooooooooooooooooooooooooooooooooooo")
  // var tran=req.params.tra;
db.transactionSeriesInvoice.find({"TransactionType":'Order Detail'},function(err,doc12){
    console.log(" hhhhhhhhhhhhhhhhhhhhhcheck datafrwsdtywefdqwetyfqewfty ");
    console.log(doc12[0]);
    console.log(doc12);
    res.json(doc12);
});
...
//for getting totalcount
app.get('/Ordertotalcount',function(req,res){
...
app.post('/AccountStatusReceipt:values',function(req,res){
  ...
  // receipts var voucher = insertbill_array[0];

  db.receipts.update({"voucherNo" : insertbill_array[0]},{$set:{"voucherStatus":"completed"}}, { multi: true },function(err,doc){
    
     // res.json(doc);
      console.log("receipts receiptsCall replayyyyyyyyyyyyyyyyyyyyy "+insertbill_array[0]);
      console.log(doc);
...
app.post('/AccountStatusPayments:values',function(req,res){
  ...
  db.payments.update({"voucherNo" : insertbill_array[0]},{$set:{"voucherStatus":"completed"}}, { multi: true },function(err,doc){
...
app.get('/lengthOrdNum/:voucher',function(req,res){
  console.log("kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk ");
  // var voucher = req.params.voucher;
  // console.log("partyname is "+ voucher);
  db.orders.find({"orderNO" : req.params.voucher,"initial":{ $ne: "Delivered" }},function(err,doc){
    res.json(doc);
  });
});
app.get('/completedLengthOrdNum/:voucher',function(req,res){
  console.log("kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk ");
  // var voucher = req.params.voucher;
  // console.log("partyname is "+ voucher);
  db.orders.find({"orderNO" : req.params.voucher,"initial":"Completed"},function(err,doc){
    res.json(doc);
  });
});
app.get('/overAllLengthOrdNum/:voucher',function(req,res){
  console.log("kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk ");
  // var voucher = req.params.voucher;
  // console.log("partyname is "+ voucher);
  db.orders.find({"orderNO" : req.params.voucher},function(err,doc){
    res.json(doc);
  });
});
...
console.log(rdata1+"uuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuu")
  ...
  db.receipts.insert({"Mode":mode,"Amount":Decimal128.fromString(amount),"Bank":bank,"ChequeNo":chequeno,"Date":chequeDate,"CardNo":cardnos,"CardType":ctype,"ApprovalNo":appno,"partyname":pname,"BilledDate":new Date(bdate),
    "BillNo":bill,"Narration":narrate,"PaidAmount":Decimal128.fromString(totals),"voucherNo":voucher,"voucherStatus":voucherStatus,"orderNO":orderNO,"remainingPaid":remainingPaid,'netBalance':Decimal128.fromString(netBalance)},function(err,doc){
...
app.get('/receipetCreation',function(req,res){   
      ...
      console.log(" BillNo  userId "+BillNo+req.query.userId+req.query.voucherNo);
...
var dis = str_array[9];
    ...
    var netamt = str_array[11];
     var invoiceValue = str_array[12];
      ...
      console.log(transaction+"transaction transaction transaction transaction transactiontransaction");
...
app.put('/changeGrwt/:update',function(req,res)
{
  
    var str=req.params.update;
    console.log(str+"lllllllllllllllllllllllllllllllllllllllllllll");
    var str_array=str.split(",");
   
    var orderNo=str_array[0];
    //console.log(partyname);
    var gwt=str_array[1];
    console.log(orderNo+gwt+"oooooooooooooooooooooooooooooooooooooooo");
    db.transactionDetail.update({"orderNo":orderNo},{$set:{"status":"orderCompleted","chgunt":gwt}},function(err,doc){
...
app.put('/putPaid/:update',function(req,res)
{
    ...
    var orderNO=str_array[0];
    //console.log(partyname);
    var paid=str_array[1];
    console.log(orderNO+paid+"oooooooooooooooooooooooooooooooooooooooo");
    db.receipts.update({"orderNO":orderNO},{$set:{"status":"orderCompleted","remainingPaid":paid}},function(err,doc){
...
//<<<<<<< HEAD
          ...
          db.receipts.aggregate([{$match:{"partyname":name,"voucherStatus" : "InProgress"}},
              {$group:{_id:{"partyname":"$partyname"},Paid:{$sum:"$Amount"}}}

              ],function(err,paid){
                   var balanceAmount = 0.00;
                    var paidAmountBalance = 0.00;
                  if (balance.length == 0) {
                    //res.json(0.00);
                    balanceAmount = 0.00
                  }else{
                  ...
                  if (paid.length == 0) {
                    //res.json(0.00);
                    paidAmountBalance = 0.00;
                  }else{
                        paidAmountBalance = paid[0].Paid ;
                  }
                   var resultValue = balanceAmount -  paidAmountBalance ;
...
//for getting receipt details
app.get('/getAppReceivableAmount:name',function(req,res){
  var name=req.params.name;
  console.log("num num num num num num num num num");
  db.saleInvoice.aggregate([{$match:{"partyname":name,"Transaction":"Approval Sale","AccountStatus":'Inprogress',"voucherNo":{$ne:"null"}}},
     { $group:{_id:{partyname:"$partyname"},Balance:{$sum:"$netAmount"} }},
// =======
// >>>>>>> 7a8f5baafd1db8333d296c8333f507e52e0c2c4e
...
//for getting receipt details
app.get('/receiptOrderNtWt/:name',function(req,res){

  var name=req.params.name;
  console.log(name+"233333323232223jjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjj")
   db.transactionDetail.find({"orderNo":name},function(err,doc){
        res.json(doc);
        //console.log(doc)
       });
...
//   console.log(pname);
//   var trans="Approval Out";
//   db.transactionDetail.aggregate([{$match:{"orderNo":pname}},
//       {$group:{_id:{partyname:"$partyname"},Balance:{$sum:"$ntwt"}}}],function(err,doc){ 
//     res.json(doc);
//     console.log(doc+"Approval Outs");
//   });
// });
...
app.get('/getpaymentReceivableAmount:name',function(req,res){
  ...
  db.saleInvoice.aggregate([{$match:{"partyname":name,"Transaction":"RD Purchase","AccountStatus":'Inprogress',"voucherNo":{$ne:"null"}}},
     { $group:{_id:{partyname:"$partyname"},Balance:{$sum:"$netAmount"} }},
          ...
          db.payments.aggregate([{$match:{"partyname":name,"voucherStatus" : "InProgress"}},
              {$group:{_id:{"partyname":"$partyname"},Paid:{$sum:"$Amount"}}}
                 ...
                 //    res.json(0.00);
                 //  }else{
                 //    res.json(balance[0].Balance - paid[0].Paid);
                 //  }
              })
  })
...
//for item html get details
app.get('/getInventoryGroupMaster:name',function(req,res)
{
   // console.log("i received a get request from index");
...
app.get('/purchasess',  function (req, res) {
  ...
  console.log("jjjjjjj")
    db.ledgeraccounts.find({accountName: { $exists: true },"grpId" : "MG2"},function(err,doc){
...
app.get('/sales',  function (req, res) {
  ...
  console.log("jjjjjjj")
    db.ledgeraccounts.find({accountName: { $exists: true },"grpId" : "MG6"},function(err,doc){
...
console.log(samedateStart +"   "+samedateEnd)
  ...
  function openingBalanceCall() {
      console.log("openingBalanceCall(1)");
      ...
      db.transactionDetail.aggregate([
         ...
         // console.log(doc);
           ...
           function compareStockInwardYes(m) {
             console.log(" compareStockInwardYes(m) "+doc1.length+"  compareStockInwardNoLength "+doc.length)
             compareStockInwardYesLength =  doc1.length;
             compareStockInwardNoLength =  doc.length;
             if(m == 0 && compareStockInwardYesLength == 0 && compareStockInwardNoLength ==0 ){
               console.log(m+" m "+"  compareStockInwardYesLength "+compareStockInwardYesLength+" compareStockInwardNoLength "+compareStockInwardNoLength)
            ...
            if (m< compareStockInwardYesLength) {
              ...
              //console.log("  compareStockInwardYesLength "+m)
              function compareStockInwardNo(n) {
                console.log(" compareStockInwardNo(n) ")
                if (n< compareStockInwardNoLength) {
                         console.log(doc1[m]._id.SaleCategory +" n "+n);
                         if (doc[n]._id.SaleCategory == doc1[m]._id.SaleCategory) {
                           //console.log(doc[n]._id.SaleCategory +" n "+n+" m "+m+ " doc1[m]._id.SaleCategory "+doc1[m]._id.SaleCategory);
                             set2.add(doc1[m]._id.SaleCategory);
                             ...
                             opgwt:doc1[m].ingwt - doc[n].ingwt ,opgpcs:doc1[m]. ingpcs - doc[n]. ingpcs , };
                              ...
                              newArray.push(obj);
                              console.log(doc[n]._id.SaleCategory +" doc[n]._id.SaleCategory == doc1[m]._id.SaleCategory "+n);
                              console.log(newArray)
                              console.log(" m 123 "+m+" n "+n); 

                         }//else{//if (doc[n]._id.SaleCategory == doc1[m]._id.SaleCategory) {
                              //  set2.add(doc1[m]._id.SaleCategory);
                              //}

                         compareStockInwardNo(n+1)
                }else{//n< compareStockInwardNoLength
                    ...
                    if (m == compareStockInwardYesLength-1 ) {
                         console.log(" m   vijay    "+m);
                          ...
                          compareStockInwardYesNoMatches(0);
...
//write here 30/3
                         function compareStockInwardYesNoMatches(d) {

                                console.log(" m vikk == compareStockInwardYesLength-1 d "+d+" compareStockInwardYesLength "+compareStockInwardYesLength);
                               if (d< compareStockInwardYesLength) {
                                  console.log(set2)
                                    if (set2.has(doc1[d]._id.SaleCategory) == false) {
                                             console.log(" set2.has(doc1[d]._id.SaleCategory) == false m == compareStockInwardYesLength-1 m == compareStockInwardYesLength-1");
                                        //console.log(set2.has(doc1[n]._id.SaleCategory) )
                                        //console.log("in side loop a "+a);
                                         var obj = {"_id":{SaleCategory:doc1[d]._id.SaleCategory},opcount:doc1[d].incount  ,
                                             opgwt:doc1[d].ingwt,opgpcs:doc1[d]. ingpcs, };
                                         
                                              newArray.push(obj);
                                              compareStockInwardYesNoMatches(d+1); 
                                              if (d == (compareStockInwardYesLength - 1)) {
                                                   if(newArray.length == 0){
                                                     res.json(newArray)
                                                   }
                                                   resolve(newArray)
                                    ...
                                    }else{// if (set2.has(doc1[n]._id.SaleCategory) == false) {
                                              console.log(" doc1[d]._id.SaleCategory "+doc1[d]._id.SaleCategory);
                                               compareStockInwardYesNoMatches(d+1); 
                                          }

                               }else{// if compareStockInwardYesNoMatches
                                       if(newArray.length == 0){
                                         res.json(newArray)
                                       }
                                       resolve(newArray)
                                    ...
                                    }//// else compareStockInwardYesNoMatches
                              
                         }//compareStockInwardYesNoMatches(m)
                         //compareStockInwardYesNoMatches(0);
...
{$match:
  ...
  function closingBalanceCall() {
    console.log(" inwardBalance closingBalanceCall(4) "+ inwardBalance.length);
      ...
      var  report1 = [];
      var balance = [];
      //changed 30/3 vijay for first date
      if (openingBalance != null) {
        var m = openingBalance.length;
     
       console.log(openingBalance.length+" openingBalance ");
                ...
                //for (var j = inwardBalance.length-1 ; j >= 0; j--) {
                     function inwardBalanceInwardNo(j) {
                          if (j>=0) {
                              var obj={}
                              console.log(" else call "+j)
                              console.log(inwardBalance)
                              obj["SaleCategory"] = inwardBalance[j]._id.SaleCategory;
                              ...
                              console.log(report1)
                               ...
                               inwardBalanceInwardNo(j-1);
                          };

                      
                     }// inwardBalanceInwardNo(j)
                     inwardBalanceInwardNo(inwardBalance.length-1)    
                              //notMatchinginwardBalance()
                                              //balance.push(openingBalance[i]._id.SaleCategory);
...
app.post('/remainingNtWt/:data',function(req,res){ 
  ...
  var str=req.params.data;
    ...
    for(var i =0;i<length - 1;i++){
    //  console.log("here is the length "+str_array[i]);
      db.transactionDetail.update({_id:mongojs.ObjectId(str_array[i])},{$set:{"voucherNo":voucher,"remainingNtWt":remainingNtWt }},function(err,doc)
        {
        //res.json(doc);
        //console.log(doc)
       });
   ...
   // console.log('look up things syuasasdyusadsdhyasdbdfhudbasjdbashudbhdhy');
    app.put('/putReceiptOrderNtWt/:data',function(req,res){
      //console.log('updating on approval sale');
      var str=req.params.data;
      var str_array=str.split(",");
      var id=str_array[0];
      var trans=str_array[1];
      console.log(id+"lllllllllllllllllllsssssssssssss")
      // var voucher=str_array[2];
      db.transactionDetail.update({"orderNo":id},{$set:{"remainingNtWt":trans}},function(err,doc){
        res.json(doc);
      })
    });
